...
\replaced[id=R3, comment={R3-Min-4b: Citation-as-noun fix}]{
...
\added[id=R2, comment={R2-5b: Regional mesh refinement discussion}]{
...
\added[id=R1, comment={R1-Maj-5b: Geometric-functional correlation discussion}]{
...
As such, the results reflect the isolated impact of 
\replaced[id=R1, comment={R1-Min-3: Structural variation wording fix}]{
    anatomical structure variation
}{
...
\replaced[id=MR, comment={R1-Maj-3, R3-Maj-2, R3-Maj-6: Validation reframe}]{
    ...
    These results demonstrate that the virtual cohort captures both healthy anatomy and 
    pathological remodelling, while also exposing the limitations of fixed reference ranges 
    that do not account for sex and disease interplay.
}

\paragraph{Impact of Sex and Heart Failure in \emph{In-Silico} Studies}
\replaced[id=MR, comment={R1-Maj-4: Geometry causality rewrite}]{
    ...
    \added[id=R3, comment={R3-Min-2: }]{
        (Figure~\ref{fig:cohen-big-effects}(a)), however, 
    }
    \HF\ females showed the greatest relative differences, 
    ...
    Our simulations consistently separated groups by sex and disease status, even in the 
    absence of tuned electrophysiological or mechanical parameters. 
    Absolute LV volume increases were particularly marked in male HF, reproducing the 
    clinical signature of pathological dilation~\cite{kasa-2024-lv_volume,ito-2021-lv_volume}. 
    ...
    Similarly, larger $\deltaVol{LA}$ would indicate other issues like increased prediction 
    of mortality or hospitalisation~\cite{Carpenito-2021-la-stiffness,kim-2023-la-stiffness}. 
    ...
    This emphasises that absolute and relative measures provide complementary insights and must 
    be interpreted carefully in sex-stratified analyses~\cite{steuernagel2023_sex_bias}. 
    Electrical activation followed similar logic: ventricular and atrial activation were prolonged 
    in HF and in males, consistent with clinical QRS and conduction 
    observations~\cite{sanders-2003-atrialremodelling,buck-2008-crt}, though interactions were not significant. 
    These convergent trends across independent metrics highlight that anatomical variability alone 
    can generate clinically relevant patterns of electromechanical behaviour, a finding consistent 
    with model-based predictions of sex-specific conduction behaviour~\cite{Lee2019_gender}. 
    These findings suggest that even without personalisation of tissue properties or conduction 
    parameters, anatomically driven whole-heart simulations can outline group-level trends in 
    cardiac function. 
    The consistent importance of sex and disease differences across multiple metrics reinforces 
    the clinical relevance of the modelling pipeline and motivates its use in larger-scale 
    studies~\cite{strocchi2020_cohort}.
}
...
\replaced[id=R1, comment={R1-Maj-6: ISCT language de-escalation}]{
    ...
    by assessing cohort representativeness (and simultaneously assessing accuracy of 
    the model generation pipeline) through comparison the ventricular and atrial volumes 
    in the male and female cohort subsets with corresponding UK BioBank data.
}
...
\added[id=MR, comment={R3-Maj-3b: TAT-QRS limitation acknowledgment}]{
...
\added[id=R3, comment={R3-Maj-7: UKBB Caucasian limitation}]{
    Additionally, the UKBB reference ranges used for anatomical comparison~\cite{petersen-2017-reference} 
    were derived from a predominantly Caucasian population. This may limit the generalizability 
    of our anatomical fidelity assessment across diverse ethnic backgrounds, as chamber sizes 
    and remodeling patterns can vary by ethnicity. Our cohort lacks systematic ethnicity data, 
    preventing stratified analysis. 
    Future work should incorporate ethnicity-specific reference ranges or population-matched 
    controls to improve the validity of anatomical comparisons across diverse patient demographics.
}
...
\added[id=R2, comment={R2-4: Geometry-image correspondence}]{
...
\added[id=R1, comment={R1-Maj-7: Uncertainty quantification discussion}]{
    While this study demonstrates that anatomical variability influences 
    simulation outcomes under fixed parameters, uncertainties in the segmentation and 
    mesh generation process may further contribute to output variance. 
    We have previously assessed reproducibility of our left atrial modeling 
    pipeline~\cite{solis-lemus2023_reproducibility}, demonstrating that operator 
    variability in segmentation introduces quantifiable uncertainty in chamber geometry 
    and subsequent simulation predictions. However, whole-heart models present 
    substantially greater complexity, with four chambers, multiple vessel junctions, 
    and valve planes that may amplify uncertainty propagation through the reconstruction 
    workflow.
}
\added[id=R1, comment={R1-Maj-7}]{
    A systematic approach to quantifying these uncertainties would involve Monte Carlo 
    sampling of plausible segmentation contours within the bounds of image resolution 
    and anatomical variability. For instance, systematically perturbing myocardial 
    boundaries by $\pm 1$--2~mm (corresponding to typical CT voxel dimensions) and 
    regenerating meshes would yield a distribution of geometries consistent with the 
    underlying imaging data. Running simulations across this ensemble would quantify 
    the sensitivity of activation times and mechanical responses to reconstruction 
    uncertainty, extending our prior single-chamber reproducibility analysis to the 
    multi-chamber whole-heart case.
}
\added[id=R1, comment={R1-Maj-7}]{
    Such uncertainty quantification analyses align with verification, validation, and 
    uncertainty quantification (VVUQ) frameworks increasingly adopted for computational 
    model credibility assessment in regulatory contexts~\cite{health_assessing_2023}. 
    Incorporating systematic whole-heart UQ would strengthen confidence in model 
    predictions and provide bounds on simulation outputs that account for both 
    anatomical variability (addressed in this study) and reconstruction uncertainty 
    (future work). This represents a critical next step toward establishing the 
    credibility of this virtual cohort for in silico clinical trial applications.
}
