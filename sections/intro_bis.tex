...
Patient-specific computational models and simulations 
\deleted[id=R3, comment={R3-Min-3a: CM\&S acronym removal}]{\CMnS}
of the heart have increasingly  
...
\replaced[id=R3, comment={R3-Min-4a: Citation-as-noun fixes}]{
...
\replaced[id=R3, comment={R3-Min-4a: Citation-as-noun fixes}]{
    ...
    For example, the~\cite{strocchi2020_cohort} database has been widely used for 
    studies on valve mechanics~\cite{feng2024_db_user}, 
    fibrosis detection~\cite{okenov2024_db_user}, and 
    arrhythmia modeling~\cite{kaboudian2023_db_user}.
}
...
of data used for \replaced[id=R3, comment={R3-Min-3: CM\&S acronym removal}]{
    computational models and simulations}{\CMnS}~\cite{steuernagel2023_sex_bias}.
...
In this study, we \replaced[id=MR, comment={R3-Min-3, R3-Min-4, R3-Min-5: Minor Editorial Corrections}]{
    developed}{present} CEMRG Heartbuilder, 
...
\deleted[id=R3, comment={R3-Maj-1b: Second-to-last paragraph streamlining}]{
...
\replaced[id=R3]{
    ...
    We applied this workflow to a cohort of 50 patients, balanced by sex, including healthy controls 
    and two heart failure subtypes (narrow and wide QRS). 
    Benchmark simulations with uniform material properties isolated the impact of anatomical differences 
    attributed to sex and disease on cardiac electrophysiology and mechanics.
}
The simulations \replaced[id=MR]{demonstrated}{demonstrate} the feasibility and robustness of the approach,
\added[id=R3]{
...
% {
%     The results \replaced[id=JASL]{demonstrated}{demonstrate} the feasibility and robustness of the approach.
% }
\deleted[id=R3, comment={R3-Maj-1a: Last paragraph deletion}]{The aim of this study is to develop a workflow for creating hearts, 
...
\replaced[id=JASL]{Figure~\ref{fig:heartbuilder_pipeline} provides an overview of the pipeline and the resulting cohort.}
{Figure~\ref{fig:heartbuilder_pipeline} provides an overview of the workflow and resulting cohort.}

\begin{figure}[hbpt]
    \centering
    \includegraphics[width=\textwidth]{pics/fig1_graphical_abstract.pdf}
