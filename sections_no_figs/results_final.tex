\section{Results}
Fifty whole-heart models (S1 Fig) were created,
...
A full table of means and standard deviations is provided in S2 Data.
...
% Hearts cohort figure relocated to supplement as S1 Fig (per reviewer request).
...
Effect sizes were computed using Cohen’s $d$ and are visualised in Fig~\ref{fig:cohen-big-effects}.
...
($[88,161]~mL$).
We display the results in Fig~\ref{fig:validation-volumes}.
...
($p<0.05$ in all cases, Fig~\ref{fig:cohen-big-effects}(a)).
...
and $d=0.78, p<0.01$ for male vs female comparisons (Fig~\ref{fig:cohen-big-effects}(b)).
