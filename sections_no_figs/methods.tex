...
deep learning model by~\cite{hao2021_ccta}, Fig~\ref{fig:methods-segmentation}a, 
...
A post-processing stage follows, increasing the number of labels to 37 (Fig~\ref{fig:methods-segmentation}b). 
...
\begin{figure}[hbpt]
    % \centering
    % \includegraphics[width=\textwidth]{pics_formatted/methods-segmentation}
    \caption{
        ...
        splits the pulmonary veins into superior and inferior.
...
Fig~\ref{fig:methods-mesh} shows the output of the meshing process from the smooth segmentation 
to the working mesh re-labelled and with the blood pools removed.

\begin{figure}[hbpt]
    % \centering
    % \includegraphics[width=\textwidth]{pics_formatted/methods-meshing}
    \caption{
...
Fig~\ref{fig:methods-model-creation} shows a visual overview of the model creation stage, 
...
layer on non-conduting tissue between the atrial and ventriclar myocardium.
Finally, the \SAN\ and the left and right ventricular fascicles are also created, 
Fig~\ref{fig:methods-model-creation}~(f.1, f.2, f.3). 
...
\begin{figure}[hbpt]
    % \centering
    % \includegraphics[width=\linewidth]{pics_formatted/methods-model-creation}
    \caption{
...
which can be consulted in S1 Text.
